...
const eventSourceARN1 = samples.sampleKinesisEventSourceArn(awsRegion, 'TEST_Stream_DEV');
const eventSourceARN2 = samples.sampleDynamoDBEventSourceArn(awsRegion, 'TEST_Table_DEV', '2017-03-08T23:59:58.102Z');
...
function createContext(streamType, sequencingRequired, sequencingPerKey, idPropertyNames, keyPropertyNames, seqNoPropertyNames, event, awsContext) {
  ...
  const context = {
    streamProcessing: {
      ...
      resolveMessageIdsAndSeqNos: (msg, rec, userRec, eventIdAndSeqNos, md5s, context) => {
        return {ids: [], keys: [], seqNos: [['eventSeqNo', eventIdAndSeqNos.eventSeqNo]]};
      },
  ...
  context.stage = 'dev';
  contexts.configureStandardContext(context, undefined, require('../default-options.json'), event, awsContext, false);
...
test('new Batch & resolveBatchKey with 1 Kinesis record', t => {
  const context = createContext('kinesis', sequencingRequired, sequencingPerKey, ['id1', 'id2'], ['k1', 'k2'], ['n1', 'n2', 'n3']);

  const shardId1 = 'shardId-000000000000';
  const eventSeqNo1 = '49545115243490985018280067714973144582180062593244200961';
  const [, rec1] = sampleKinesisMessageAndRecord(shardId1, eventSeqNo1, eventSourceARN1, '123', '456', 'ABC', 10, 1, 2, 3);
...
test('new Batch with 1 DynamoDB record', t => {
  const context = createContext('dynamodb', sequencingRequired, sequencingPerKey, [], [], []);

  const eventID1 = 'E001';
  const eventSeqNo1 = '10000000000000000';
  const [, rec1] = sampleDynamoDBMessageAndRecord(eventID1, eventSeqNo1, eventSourceARN2, '123', 456, 'ABC', 10, 1, 2, 3);

  const expectedKey = {
    streamConsumerId: 'D|TEST_Table_DEV/2017-03-08T23:59:58.102|my-function:dev',
    shardOrEventID: 'E|E001'
  };
  ...
  t.equal(components.streamName, streamName, `components.streamName must be ${streamName}`);
  t.equal(components.consumerId, consumerId, `components.consumerId must be ${consumerId}`);
  t.equal(components.shardId, undefined, `components.shardId must be ${undefined}`);
  t.equal(components.eventID, eventID1, `components.eventID must be ${eventID1}`);
...
test('new Batch & resolveBatchKey with 1 unusable record & 2 messages', t => {
  ...
  const [msg2, rec2] = sampleKinesisMessageAndRecord(shardId2, eventSeqNo2, eventSourceARN1, '123', '456', 'ABC', 10, 1, 2, 3);

  const shardId3 = 'shardId-000000000003';
  const eventSeqNo3 = '49545115243490985018280067714973144582180062593244200963';
  const [msg3, rec3] = sampleKinesisMessageAndRecord(shardId3, eventSeqNo3, eventSourceARN1, '456', '789', 'DEF', 11, 4, 5, 6);
...
test(`reviveTasks with 0 messages and 2 unusable records AND sequencingRequired ${sequencingRequired}`, t => {
  ...
  const [, rec2] = sampleKinesisMessageAndRecord(shardId2, eventSeqNo2, eventSourceARN1, '123', '456', 'ABC', 10, 1, 2, 3);
...
test(`reviveTasks with with 2 ok messages and 1 rejected message AND sequencingRequired ${sequencingRequired}`, t => {
  ...
  const eventSeqNo1 = '49545115243490985018280067714973144582180062593244200961';
  const [msg1, rec1] = sampleKinesisMessageAndRecord(shardId1, eventSeqNo1, eventSourceARN1, '123', '455', 'ABC', 10, 1, 2, 3);

  const shardId2 = 'shardId-000000000002';
  const eventSeqNo2 = '49545115243490985018280067714973144582180062593244200962';
  const [msg2, rec2] = sampleKinesisMessageAndRecord(shardId2, eventSeqNo2, eventSourceARN1, '123', '456', 'ABC', 10, 4, 5, 6);

  const shardId3 = 'shardId-000000000003';
  const eventSeqNo3 = '49545115243490985018280067714973144582180062593244200963';
  const [msg3, rec3] = sampleKinesisMessageAndRecord(shardId3, eventSeqNo3, eventSourceARN1, '456', '789', 'DEF', 11, 7, 8, 9);
...
test(`abandonDeadProcessingTasks`, t => {
  ...
  const [msg1, rec1] = sampleKinesisMessageAndRecord(shardId, eventSeqNo1, eventSourceARN1, '123', '456', 'ABC', 10, 1, 2, 3);
  const [msg3, rec3] = sampleKinesisMessageAndRecord(shardId, eventSeqNo3, eventSourceARN1, '123', '456', 'ABC', 10, 1, 2, 4);
...
// =====================================================================================================================
// getState & getOrSetState
// =====================================================================================================================

test('getState & getOrSetState', t => {
  const context = createContext('dynamodb', sequencingRequired, sequencingPerKey, ['id1', 'id2'], ['k1', 'k2'], ['n1', 'n2', 'n3']);
  ...
  const [msg2, rec2] = sampleDynamoDBMessageAndRecord(eventID2, seqNo2, eventSourceARN2, '123', '456', 'ABC', 10, 1, 2, 3);
